...
const Search = () => {
    ...
    useEffect(() => {
        const loadProducts = async () => {
            ...
            try {
                const { data } = await axiosClient.get(`/search/${q}?page=${page}&limit=${limit}&minprice=${minPrice}&maxprice=${maxPrice}`);
    ...
    return (
        <>
            <div className="mx-auto max-w-7xl sm:px-6 sm:py-24 lg:px-8">
                <div className="font-extrabold text-2xl hover:underline">Resultats de Recherche <span className="text-amber-700">{q}</span></div>
                ...
                <div className=" text-md font-semibold flex">
                    <p className="text-gray-700 font-semibold">Filtrer</p>
